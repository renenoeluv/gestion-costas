...
CREATE TABLE IF NOT EXISTS `sgc`.`SGC_PLAYA` (
  ...
  `PLAT_LATITUD` DECIMAL(9,6) NULL,
  `PLAT_LONGITUD` DECIMAL(9,6) NULL,
  `SGC_CIUDAD_CIU_ID` INT NOT NULL,
  `PLA_CODIGO_WINDGURU` VARCHAR(25) NULL,
  `PLA_PENDIENTE` DECIMAL(2,2) NOT NULL,
  PRIMARY KEY (`PLA_ID`),
...
CREATE TABLE IF NOT EXISTS `sgc`.`SGC_PERFIL` (
  ...
  `PLA_ID` INT NOT NULL,
  `PERF_REPRESENTATIVO` VARCHAR(1) NOT NULL,
  INDEX `PERF_NUMERO_idx` (`PERF_NUMERO` ASC),
...
CREATE TABLE IF NOT EXISTS `sgc`.`SGC_CAMPANHA` (
  ...
  `CAMP_TAMANHO_MEDICION` DECIMAL(4,2) NOT NULL,
  `CAMP_ESTADO` VARCHAR(45) NOT NULL,
  `SGC_USUARIO_ENCARGADO` INT NOT NULL,
  `PLA_ID` INT NOT NULL,
  PRIMARY KEY (`CAMP_ID`),
  INDEX `SGC_CAMPANHA_idx` (`CAMP_ID` ASC),
...
CREATE TABLE IF NOT EXISTS `sgc`.`SGC_BITACORA` (
  ...
  `BIT_ESTADO` VARCHAR(10) NOT NULL,
  `BIT_MAXIMA` VARCHAR(1) NULL,
  PRIMARY KEY (`BIT_ID`),
...
CREATE TABLE IF NOT EXISTS `sgc`.`SGC_MAREA` (
  ...
  `MARE_ALTURA` DECIMAL(4,2) NOT NULL,
  `SGC_CIUDAD_CIU_ID` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `user` ( `id` int(12) NOT NULL AUTO_INCREMENT, `fname` varchar(30) NOT NULL, `lname` varchar(30) NOT NULL, `email` varchar(60) NOT NULL, `password` varchar(40) NOT NULL, PRIMARY KEY (`id`), UNIQUE KEY `email` (`email`) ) ENGINE=MyISAM DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
